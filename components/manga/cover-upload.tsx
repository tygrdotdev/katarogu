...
export default function MangaCoverUpload(props: ButtonProps & { setValue: (value: string) => void }) {
        ...
        if (e.target.files && e.target.files.length > 0) {
            ...
            image.onload = () => {
                if (image.width < 200 || image.height < 400) {
                    resetCrop();
                    return toast.error("Please use an image that is at least 200x400 pixels.");
                }
                if (!["image/jpeg", "image/png", "image/webp"].includes(type)) {
                    resetCrop();
                    return toast.error("Images must be in either JPEG, PNG, or WEBP format.");
                }
                else {
                    setOpen(true);
                }
    ...
    if (isDesktop) {
        return (
            <>
                ...
                <input ref={imageInputRef} type="file" accept="image/jpeg, image/png, image/webp" className="hidden" onChange={onSelectFile} />
