                            ...
                            <Link href={`/manga/${manga.expand?.entry.id}`} passHref key={manga.expand?.entry.id}>
                                <div className="flex border-black/10 dark:border-white/10 rounded-md w-full h-fit gap-2 flex-col group">
                                    <AspectRatio ratio={2 / 3} className="w-full h-full rounded-md overflow-hidden">
                                        <Image src={`${process.env.NEXT_PUBLIC_AUTH_URL}/api/files/manga/${manga.expand?.entry.id}/${manga.expand?.entry.cover}`} alt={manga.expand?.entry.title + " cover"} width={150} height={300} className="w-full h-full rounded-md group-hover:scale-105 transition-all  border-black/10 dark:border-white/10" />
                                    </AspectRatio>
